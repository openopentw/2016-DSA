...
bool row_change[40], col_change[40];
...
void parsing_row(int row_th, VIit row_beg, VIit row_end, int map_beg, int map_end)
{
	for(int i = map_beg; i < map_end; ++i)
		if(map[row_th][i] == '#') {
			int j;
			for(j = i + 1; j < map_end && map[row_th][j] != '.'; ++j) ;
			if(j - i == *row_beg) {
	...
	for(int i = map_end; i > map_beg; --i)
		if(map[row_th][i] == '#') {
			int j;
			for(j = i - 1; j > map_beg && map[row_th][j] != '.'; --j) ;
			if(i - j == *row_end) {
...
int judge_row(int row_th, int col_end)
{
	...
	// for(VIit it = row[row_th].line.begin(); it != row[row_th].line.end(); ++it) {
		...
		// if(col_end != col_n - 1 && col_th == col_end) {
			// if(len > *it)	return 0;
		// } else if (len != *it) {
			// return 0;
		// }
	...
	int count = -1;
	VIit it = row[row_th].line.begin();
	for(col_th = 0; col_th <= col_end; ++col_th) {
		while(col_th <= col_end && map[row_th][col_th] == '.')	++col_th;
		if(col_th > col_end)
			return 1;
		int len = 0;
		while(col_th <= col_end && map[row_th][col_th] == '#')	++col_th, ++len;
		count++;
		if(count > row[row_th].line.size())	return 0;
		else {
			if(col_end != col_n - 1) {
				if(col_th == col_end) {
					if(len > *it)	return 0;
...
int judge_col(int col_th, int row_end)
{
	...
	// for(VIit it = col[col_th].line.begin(); it != col[col_th].line.end(); ++it) {
		...
		// if(row_end != row_n - 1 && row_th == row_end) {
			// if(len > *it)	return 0;
		// } else if (len != *it) {
			// return 0;
		// }
	...
	int count = -1;
	VIit it = col[col_th].line.begin();
	for(row_th = 0; row_th <= row_end; ++row_th) {
		while(row_th <= row_end && map[row_th][col_th] == '.')	++row_th;
		if(row_th > row_end)
			return 1;
		int len = 0;
		while(row_th <= row_end && map[row_th][col_th] == '#')	++row_th, ++len;
		count++;
		if(count > col[col_th].line.size())	return 0;
		else {
			if(row_end != row_n - 1) {
				if(row_th == row_end) {
					if(len > *it)	return 0;
...
int judge(int row_th, int col_th)
{
	if(judge_row(row_th, col_th) && judge_col(col_th, row_th))	return 1;
	return 0;
}
int run_step = 0;
int fill_OX(int step)
{
	...
	if (step == row_n * col_n)
		return 1;
	// if(run_step % 1 == 0)	printmap_debug();
	...
	if(map[row_th][col_th] == '#' || map[row_th][col_th] == '.') {
		// map[row_th][col_th] = map[row_th][col_th];
