...
int judge(int row_th, int col_th)
{
	...
	int i = 0;
	while(i <= col_th) {
		while(i <= col_th && map[row_th][i] == '.')	++i;
		if(i == col_th)	break;
		int len = 0;
		while(i <= col_th && map[row_th][i] == '#')	++i, ++len;
		if (it == row[row_th].line.end()) {
			for (int j = i;j <= col_th;j++)
				if (map[row_th][j] == '#')	return 0;
		}
		if(col_th != col_n - 1) {
			if(i == col_th) {
				if(len > *it)	return 0;
	...
	it = col[col_th].line.begin();
	i = 0;
	while(i <= row_th) {
		while(i <= row_th && map[i][col_th] == '.')	++i;
		if(i == row_th)	break;
		int len = 0;
		while(i <= row_th && map[i][col_th] == '#')	++i, ++len;
		if (it == col[col_th].line.end()) {
			for (int j = i;j <= row_th;j++)
				if (map[j][col_th] == '#')	return 0;
		}
		if(row_th != row_n - 1) {
			if(i == row_th) {
				if(len > *it)	return 0;
...
int fill_OX(int step)
{
	...
	if(inimap[row_th][col_th] == '#' || inimap[row_th][col_th] == '.') {
		...
		if(judge(row_th, col_th))	if(fill_OX(step + 1))	return 1;
		// if(run_step < 100)	printmap_debug();
		map[row_th][col_th] = '.';
		if(judge(row_th, col_th))	if(fill_OX(step + 1))	return 1;
	}
