	...
	// for(char c = '2'; c < '3'; ++c)
	{
		...
		while(int fend = fread(buf, sizeof(char), NCHAR, fp))
		{
			...
			while(end < fend)
			{
				...
				int len = 0;
				for( ; buf[end] != '\t'; ++end, ++len) {
					dictable[nline].key += dictable[nline].key * multi + buf[end] - 'a' + add;
					dictable[nline].str[len] = buf[end];
				}
				dictable[nline].key %= NHASH;
				dictable[nline].str[len] = '\0';
				// string str(buf + beg, buf + end);
				++end;	// buf[end] == '\t'
				
				// unsigned freq = 0;
				for(; buf[end] != '\n'; ++end) {
					dictable[nline].freq = dictable[nline].freq * 10 + buf[end] - '0';
				}
				++end;	// buf[end] == '\n'
